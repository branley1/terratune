...
const Sidebar = ({ isVisible, onToggle }) => {
  return (
    <aside 
      className={`fixed left-0 top-0 bottom-0 z-50 flex flex-col bg-black/50 backdrop-blur-xl border-r border-white/5 transition-all duration-300 ease-in-out ${isVisible ? 'w-60 p-6' : 'w-20 py-6 px-2.5'}`}
    ...
    >
      ...
      <div className={`playlists flex-grow overflow-y-auto overflow-x-hidden border-t border-white/10 pt-5 ${!isVisible ? 'opacity-0 pointer-events-none' : 'opacity-100'} transition-opacity duration-200 ease-in-out`}>
        {isVisible && (
          <>
            <button className="flex items-center text-sm font-semibold text-gray-400 hover:text-white mb-4 w-full py-1">
            ...
            <NavLink to="/collection/tracks" className={({isActive}) => `flex items-center text-sm font-semibold mb-4 w-full py-1 ${isActive ? 'text-white' : 'text-gray-400 hover:text-white'}`}>
            ...
            <h3 className="text-xs uppercase tracking-wider text-gray-500 mb-2.5 whitespace-nowrap px-0">Playlists</h3>
            <ul className="space-y-1">
              <li><a href="#" className="text-sm text-gray-400 hover:text-white block truncate py-1.5">Forest Chill</a></li>
              <li><a href="#" className="text-sm text-gray-400 hover:text-white block truncate py-1.5">Mountain Highs</a></li>
              <li><a href="#" className="text-sm text-gray-400 hover:text-white block truncate py-1.5">Ocean Calm Radio</a></li>
              <li><a href="#" className="text-sm text-gray-400 hover:text-white block truncate py-1.5">Rainy Day Focus</a></li>
              <li><a href="#" className="text-sm text-gray-400 hover:text-white block truncate py-1.5">Desert Winds</a></li>
              <li><a href="#" className="text-sm text-gray-400 hover:text-white block truncate py-1.5">Jungle Beats</a></li>
            </ul>
          </>
        )}
      </div>
